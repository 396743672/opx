...
- **页面设计必须使用 ui-ux-pro-max** — 新页面或 UI 改动前，必须先调用 ui-ux-pro-max skill 分析设计系统，遵循其配色、排版、间距、交互规范。
- **分支开发** — 所有功能开发、UI 修改、Bug 修复必须创建新分支上进行，禁止直接在主分支修改代码。
- **合并即删分支** — 要求合到主分支时，默认视为同时要求删除除dev外的开发分支（本地 + 远程）。无需额外说明。
