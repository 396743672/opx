...
- 现代化简约风格，支持明暗主题
- **左侧边栏可折叠**（用户确认选择），折叠后只显示图标，节省空间
- 底部状态栏常驻显示关键系统信息（CPU、内存、软件目录）
...
| 组件库 | **Naive UI** | 开箱即用的完整 Vue 3 组件库 |
| 图标 | **@iconify/vue** (离线) | 丰富图标集，离线可用 |
...
| 选项 | 用户选择 |
|------|----------|
| 组件库 | **Naive UI**（开箱即用的完整组件库）|
| 图标方案 | **@iconify/vue 离线**（离线使用）|
| SpringBoot 启动 | **保持分组依赖设计**（拓扑排序 + 循环依赖检测）|
| 侧边栏 | **可折叠侧边栏**（折叠后只显示图标）|
| 关闭窗口行为 | ✅ 三种都保留：最小化到托盘 / 直接退出 / 后台服务 |

### 修正说明

只将 **OPX 自身** 注册为系统服务，由 OPX 统一按配置顺序启动管理的软件和应用，不需要给每个软件单独注册系统服务。
