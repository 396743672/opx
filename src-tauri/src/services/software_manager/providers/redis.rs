...
mod tests {
    ...
    #[test]
    fn redis_start_command_uses_relative_config_path() {
        ...
        // 使用相对路径 "redis.conf"（Cygwin 不认 Windows 绝对路径）
        assert_eq!(cmd.args, vec!["redis.conf".to_string()]);
    ...
    #[test]
    fn redis_start_command_defaults_to_config_file() {
        let p = RedisProvider::new();
        let ctx = super::StartContext {
            installed_id: "uuid".to_string(),
            install_path: "apps/redis/7.4.9".to_string(),
        ...
        let cmd = p.start_command(&ctx).unwrap();
        // 无论 config 是否为空，都使用配置文件启动
        assert_eq!(cmd.args.len(), 1);
        assert!(cmd.args[0].ends_with("redis.conf"));
    }
