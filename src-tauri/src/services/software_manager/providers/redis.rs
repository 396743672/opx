...
impl SoftwareProvider for RedisProvider {
    ...
    fn post_install(&self, ctx: &InstallContext) -> Result<()> {
        // 生成 OPX 定制的 redis.conf，覆盖 zip 包自带的默认配置文件。
        // zip 自带的 redis.conf 含 bare `requirepass`（无值）等语法错误，会导致启动失败。
        // 后续用户通过配置表单修改后，write_form_to_config 会以 KeyValue 格式写入。
        let conf_path = ctx.install_dir().join("redis.conf");
        let content = format!(
            "# Redis configuration generated by OPX\n\
             # This file uses key-value format (key<space>value).\n\
             port 6379\n\
             bind 127.0.0.1\n\
             maxmemory 256mb\n\
             maxmemory-policy noeviction\n\
             # requirepass (no password by default)\n"
        );
        // 强制覆盖 zip 自带的 redis.conf（post_install 仅在安装时执行一次）
