export default {
  ...
  listeningPorts: '监听端口',
  localIp: '本机 IP',
  localIpHint: '留空自动探测真实网卡 IP（剔除 VMware/VirtualBox 等虚拟网卡）；填写则强制 Spring Boot 使用该 IP',
  ...
  exportConfig: '导出应用',
  importConfig: '导入应用',
  importConfigConfirm: '将导入应用（含 JAR 与应用目录，不含日志）与全局配置：同名应用会被覆盖更新，新应用将被创建。确定继续？',
  exportSuccess: '导出成功',
  importSuccess: '导入成功',
  exportProgressPreparing: '正在导出…',
  exportProgress: '正在导出 {cur}/{total}：{name}',
  exportFailed: '导出失败：{msg}',
  exportCreateFileFailed: '无法创建导出文件，请检查目标目录的写入权限与磁盘空间',
  exportWriteFailed: '写入导出包失败，请检查磁盘空间是否充足',
  exportAppFailed: '打包应用失败，导出包可能不完整',
  importProgressPreparing: '正在导入…',
  importExtracting: '正在解压…',
  importApplying: '正在应用配置…',
  importFailed: '导入失败：{msg}',
  importTmpDirFailed: '无法创建临时目录，请检查磁盘空间',
  importReadFailed: '无法读取所选文件，请确认文件存在且未被其他程序占用',
  importZipInvalid: '所选文件不是有效的导出包（zip 结构损坏或格式不符）',
  importExtractFailed: '解压导出包失败，请确认磁盘空间充足且目标目录可写',
  importManifestMissing: '导出包缺少 manifest.json，不是完整的 OPX 导出包',
  importManifestInvalid: '导出包中的 manifest.json 格式错误',
  importAppDataFailed: '导出包中的应用数据格式错误',
  importGroupFailed: '导出包中的分组数据格式错误',
  importEnvFailed: '导出包中的环境变量格式错误',
  upgradeTo: '升级到 {v}',
  ...
  elasticsearchOfficial: 'Elasticsearch 官方',
  influxdbOfficial: 'InfluxDB 官方',
  ...
  jvmStatusHeapCritical: '堆内存接近上限',
  jvmStatusFullGc: '已发生 {n} 次 Full GC',
  jvmAutoRefresh: '每 5 秒自动刷新',
  jvmHeapUsedPct: '已用 {pct}%',
  jvmHeapTrend: '堆内存趋势（最近 {n} 分钟）',
  jvmTrendAgo: '{n} 分钟前',
  jvmTrendNow: '现在',
  stopTimeout: '停止等待时长（秒）',
  stopTimeoutHint: '发出优雅停止请求后等待应用自行退出的上限，默认 30 秒（对齐 Spring Boot 的 stop 阶段超时）。Windows 上 opx 用 JDK 的 attach 机制让 JVM 自行退出（走完整 shutdown hook），应用无需任何配置，也不依赖 Actuator 端点——代价是需要本机已装 JDK，任意版本皆可',
  jarNeedsJdk: '需 Java {n}+',
  jarNoSpringBootInfo: '未识别到 Spring Boot 版本（可能不是 Spring Boot 打包）',
  jdkTooOldForJar: '所选运行时低于该应用要求，无法保存',
  jreNoJvmMonitor: '当前选的是 JRE：启动没问题，但 opx 的 JVM 监控（jstat/jcmd）只有 JDK 提供，会不可用',
  jdkTooOldBlockedSave: '所选运行时是 {cur}，低于该应用的要求（需 {n}+），保存后启动会直接失败。请改选 {n} 或更高版本的 JDK / JRE。',
  jarCompatRange: '官方兼容 Java {min}–{max}',
  jdkAboveRangeForJar: '所选 Java {cur} 超出该应用官方兼容上限（Java {max}）：官方未测试，可能因第三方库不认新字节码而启动失败。仍可保存。',
  jarBuiltWithJdkLower: '该 jar 由 JDK {b} 构建，所选 Java {cur} 更低，可能无法启动；建议搭配 Java {b} 或更高。',
  jarBuiltWithJdkHigher: '该 jar 由 JDK {b} 构建，所选 Java {cur} 更高，可以运行；如需与构建环境一致，建议搭配 Java {b}。',
  gcRecommended: '（推荐）',
  gcUnsupportedVersion: '（该 JDK 版本不支持）',
  gcUnsupportedVendor: '（该发行版不含）',
  gcUnsupportedSelectedVersion: '所选 Java {n} 未引入 {gc}，启动会直接失败（Unrecognized VM option），请改选其他 GC 类型或换更高版本的 JDK。',
  gcUnsupportedSelectedVendor: '所选运行时是 Oracle 构建，Oracle 的任何版本都不包含 {gc}（换版本也没用），启动会直接失败（Unrecognized VM option）。请改选其他 GC 类型，或换用 Temurin / Zulu / Corretto / Liberica 等 OpenJDK 发行版。',
  gcHintJdk8: 'JDK 8–10 推荐 G1GC（4G 以上堆划算），吞吐优先可选 ParallelGC；ZGC 与 ShenandoahGC 在这些版本上不存在，选中会直接启动失败',
  gcHintJdk11: 'JDK 11–16 推荐 G1GC（JVM 自身默认）；ZGC 此时仍是实验特性，不建议生产使用',
  gcHintJdk17: 'JDK 17–20 推荐 G1GC（JVM 自身默认、通用最稳）；确有极低停顿需求可显式改选 ZGC',
  gcHintJdk21: 'JDK 21–23 推荐 ZGC（分代模式，需 UseZGC + ZGenerational，停顿可压到 1ms 内）；吞吐优先可改回 G1GC',
  gcHintJdk24: 'JDK 24+ 推荐 ZGC（分代已是默认，无需额外开关）；吞吐优先可改回 G1GC',
  stoppingElapsed: '已等待 {n}s',
  jvmThreadPeak: '峰值线程',
  jvmThreadDaemon: '守护线程 {n}',
  jvmThreadStarted: '累计启动 {n}',
  jvmYoungGc: '年轻代 GC',
  jvmFullGc: 'Full GC',
  jvmGcAccum: '累计 {t}',
  jvmGcAvg: '均 {t}',
  jvmClasses: '类 {loaded} 已加载 · {unloaded} 已卸载',
  ...
  nginxConfInvalid: 'nginx 配置校验未通过，请检查站点配置后重试',
  nginxReloadFailed: 'nginx 重新加载失败，请检查 nginx 运行状态',
  certDomainInvalid: '域名不合法，请检查后重试',
  ...
  exitDirectly: '直接退出程序',
  about: '关于',
  aboutDescription: 'OPX 是一款本地软件运维管理工具：一键安装、启动、监控与备份常用中间件和服务，无需手写脚本。',
  aboutRepo: 'GitHub 仓库',
  aboutTech: '技术栈',
  checkUpdate: '检查更新',
  proxySettings: '下载代理',
  ...
  testedAt: '测试于 {time}',
  zonesCached: '已缓存域名',
  accountInUse: '该账号仍被 {n} 个站点引用，请先改掉站点配置',
  dnsAccount: 'DNS 账号',
  ...
  configField: {
    ...
    nacosLocalIp: '本机 IP',
    nacosLocalIpDesc: 'Nacos 绑定的本机 IP；留空自动探测真实网卡（剔除虚拟网卡），用于服务发现 / gRPC 地址。虚拟网卡 IP 会导致其他机器不可达',
    ...
    consulMode: '运行模式',
    consulModeDesc: 'dev 开发模式（单节点、零配置、启动即用，数据不持久化）/ server 单节点服务端模式（数据写入 data 目录持久化）',
    consulBind: '绑定地址',
    consulBindDesc: '集群通信与 HTTP API 的绑定地址（单机用 127.0.0.1；填 0.0.0.0 会同时对外暴露 API）',
    consulHttpPort: 'HTTP 端口',
    consulHttpPortDesc: 'HTTP API 监听端口（默认 8500），健康检查与 Web UI 均走该端口',
  ...
  startupReportEmpty: '本次启动没有自启项',
  startupRolledBack: '启动失败，已按逆序回滚本次拉起项',
  startupSkipped: '已跳过',
  startupKindSoftware: '软件',
  startupKindNode: '应用',
  startupKindStack: '服务组',
  startupTotalElapsed: '总耗时',
  totalElapsed: '整组耗时',
  failed: '失败',
  selectFile: '选择文件',
  ...
  updateCheckInAbout: '手动检查与安装更新请在「关于」页操作',
  checkingUpdate: '检查中…',
