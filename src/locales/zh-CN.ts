export default {
  ...
  gracefulStopUrlHint: '留空则按端口自动推导（应用若配了 server.servlet.context-path，需在此填全路径，如 http://127.0.0.1:9999/admin/actuator/shutdown）。需应用暴露 shutdown 端点，否则会直接强制终止而不再空等：2.x～3.3 用 management.endpoint.shutdown.enabled=true；3.4 起改用 management.endpoint.shutdown.access=unrestricted（4.x 已移除 enabled）。两者都要把 shutdown 加进 management.endpoints.web.exposure.include',
  gracefulStopUrlNoPort: '未填端口，将跳过优雅停止请求',
  jarNeedsJdk: '需 JDK {n}+',
  jarNoSpringBootInfo: '未识别到 Spring Boot 版本（可能不是 Spring Boot 打包）',
  jdkTooOldForJar: '所选 JDK 低于该应用要求，启动会失败',
