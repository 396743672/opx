export default {
  ...
  stopTimeoutHint: 'How long to wait for the app to exit on its own after a graceful stop request; default 30s (matches the Spring Boot shutdown phase timeout)',
  gracefulStopUrl: 'Graceful stop URL',
  gracefulStopUrlHint: 'Leave empty to derive it from the app port (if the app sets server.servlet.context-path, enter the full URL here, e.g. http://127.0.0.1:9999/admin/actuator/shutdown). The app must expose the shutdown endpoint, otherwise it is force-killed without waiting. Use management.endpoint.shutdown.enabled=true on 2.x–3.3; from 3.4 use management.endpoint.shutdown.access=unrestricted (enabled is removed in 4.x). Both need shutdown in management.endpoints.web.exposure.include',
  gracefulStopUrlNoPort: 'No port configured, the graceful stop request will be skipped',
  jarNeedsJdk: 'needs JDK {n}+',
  jarNoSpringBootInfo: 'No Spring Boot version found (not a Spring Boot repackaged jar?)',
  jdkTooOldForJar: 'the selected JDK is below this app requirement, startup will fail',
